...
export default class PhaserGameEngine {
  ...
  init(parent) {
    ...
    class GameScene extends Phaser.Scene {
      ...
      preload() {
        // Load item sprites defined in the phase configuration.
        phaseConfig.items.forEach((item) => {
          this.load.image(item.key, item.spriteUrl);
        });
        // Generic placeholder used by the confetti emitter.
        this.load.image('missing', '/assets/images/missing.png');
        // Load sound effects. These files must exist in public/assets/audio.
        // If they are missing, Phaser will ignore the load and sound playback
        // will be skipped (no error will be thrown). The files are not
        // distributed here; see README in public/assets/audio for details.
      ...
      create() {
        ...
        // Create audio objects after sounds are loaded. If the keys are not
        // found in the cache (because the files are missing), these will
        // evaluate to null and we should guard before playing.
        this.sfxSuccess = this.sound.get('sfx_success') || this.sound.add('sfx_success');
        this.sfxError   = this.sound.get('sfx_error')   || this.sound.add('sfx_error');
      ...
      handleTap(sprite) {
        ...
        if (isAllergen) {
          ...
          if (this.sfxError && this.sfxError.isDecoded) {
            try {
              this.sfxError.play();
            } catch (e) {
          ...
          if (this.sfxSuccess && this.sfxSuccess.isDecoded) {
            try {
